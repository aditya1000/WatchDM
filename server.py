...
rsf = joblib.load("C:/Aditya/matt/RF_compressed.joblib")
...
def predict():
    ...
    data_unseen = pd.DataFrame(np.reshape(final, (1,11)), columns = cols)
    prediction = rsf.predict(data_unseen)
    prediction = float(prediction)
    return render_template('results.html',pred='Expected WatchDM Score will be {}'.format(prediction))
